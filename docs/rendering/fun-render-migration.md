...
| `fun-lux` / `fun_lux` | Lighting/GI policy descriptors, light DB API, no-op Lux core, ECS resources/systems for light/emissive/GI/shadow participant extraction and diagnostics, and shadow policy decisions separate from renderer storage. | Direct lighting, many-light sampling, virtual shadow policy, GI, reflections, denoising/reconstruction, radiance/surface/probe caches. | Exists as real crate; now exposes baseline no-op frame and shutdown reports for bridge boot. Pass 13 adds `shadow::ShadowPolicyEngine` for caster selection, quality tier, soft-shadow mode, reconstruction mode, directional clipmap budget, local-light shadow budget, and policy diagnostics. | `many_light`, `virtual_shadows`, `hybrid_gi`; no default features; old `fun_lux_*` names are aliases. | `cargo check -p fun-lux`; `cargo test -p fun-lux --lib`; `cargo test -p fun-lux --lib shadow`. |
...
| Lighting/Lux seams | `fun-lux/src/{lib.rs,api.rs,shadow.rs}` | `fun-lux` | `fun-lux` | Buildable substrate, ECS extraction/update hooks, and virtual-shadow policy decisions separate from renderer page storage. |
