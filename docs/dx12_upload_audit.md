...
- `FUN_RENDER_UPLOAD_COUNTERS=1` or `RETIRED_ENGINE_RENDER_UPLOAD_COUNTERS=1` enables
  RetiredEngine `RenderQueue` counters for `write_texture`, `write_buffer`, and
  `write_buffer_with`.
...
```text
rg -n "\.write_texture\(|\.write_buffer\(|\.write_buffer_with\(|RenderQueue::write_texture|RenderQueue::write_buffer|Queue::write_texture|Queue::write_buffer|write_buffer_range" fun retired_engine avian retired_engine_quinnet thunder -g "*.rs"
```
...
|---|---|---|
| NATIVE_UI/UI large texture | `fun/game_client/src/native_ui.rs` | CPU paint fallback writes full or dirty BGRA texture rects. Semantic labels are `native_ui.cpu_paint.full_frame` and `native_ui.cpu_paint.dirty_rect`. |
| asset load / dynamic image | `retired_engine_render/src/texture/gpu_image.rs`, `retired_engine_pbr/src/render/mesh.rs` | Image asset upload and fallback texture setup. Often startup or asset-load, but dynamic image replacement can be hot. |
| per-frame generic buffer helpers | `retired_engine_render/src/render_resource/{buffer_vec,uniform_buffer,storage_buffer,sparse_buffer_vec,gpu_array_buffer,batched_uniform_buffer}.rs` | Main abstraction layer for many engine uploads. Counter callsites identify concrete helper source locations when active. |
| meshlet / visibility | `retired_engine_pbr/src/meshlet/resource_manager.rs`, `retired_engine_render/src/batching/gpu_preprocessing.rs`, `retired_engine_render/src/batching/no_gpu_preprocessing.rs` | Per-frame visibility, instance, indirect, and meshlet table uploads. Existing diagnostics already count full/range meshlet writes; upload counters add byte totals. |
| clustered lighting / decals / probes | `retired_engine_pbr/src/cluster`, `retired_engine_pbr/src/render/light.rs`, `retired_engine_pbr/src/decal/clustered.rs`, `retired_engine_pbr/src/light_probe/generate.rs` | Mostly per-frame small/medium buffers and probe generation constants. |
| Solari scene / realtime | `retired_engine_solari/src/scene/binder.rs`, `retired_engine_solari/src/realtime/{prepare,node}.rs` | Scene bind data, realtime pass constants, and denoiser/guide parameters. |
| UI render buffers | `retired_engine_ui_render/src/{lib,gradient,box_shadow,ui_material_pipeline,ui_texture_slice_pipeline}.rs` | Per-frame RetiredEngine UI vertex/index buffers; should be near-zero for the NATIVE_UI/Svelte UI path except diagnostics/FPS. |
| post-process / atmosphere / SMAA | `retired_engine_post_process`, `retired_engine_pbr/src/atmosphere`, `retired_engine_anti_alias/src/smaa` | Mostly constants and lookup buffers, usually small. |
| debug/dev tools | `retired_engine_dev_tools`, examples, screenshots/readback | Should not dominate product benchmark lanes unless diagnostic overlays are enabled. |
...
Current local DX12 auto-no-vsync evidence points at RetiredEngine generic
`DynamicUniformBuffer` and `RawBufferVec` helper rows before any `fun_render`
semantic owner is known. Those rows need semantic label splitting or a RetiredEngine
prepare/upload scheduling change before a hot owner can be moved safely.

Priority order:

1. NATIVE_UI CPU texture uploads when the accelerated DX12 lane is unavailable or
   unhealthy.
...
- Add a persistent texture-upload ring for unavoidable CPU texture uploads after
  the NATIVE_UI accelerated path and dirty-rect behavior are measured in the same
  benchmark lane.
...
Do not force RetiredEngine prepare-stage `RenderQueue::write_buffer` helpers through it
...
| `imported` | NATIVE_UI shared textures, swapchain resources, vendor SDK resources |
...
generic RetiredEngine prepare-stage writes until their semantic owners are split.
...
| DX12/native assumptions | none directly; callers pass RetiredEngine/wgpu buffers and an existing command encoder |
...
|---:|---|---|---|
| 1 | `DynamicUniformBuffer` at `uniform_buffer.rs:311` | RetiredEngine generic render resource | split semantic owner before upload-arena migration |
| 2 | `RawBufferVec` at `buffer_vec.rs:183` | RetiredEngine generic render resource | batch or split semantic owner before upload-arena migration |
| 3 | `DynamicUniformBuffer` at `uniform_buffer.rs:140` | RetiredEngine generic render resource | split semantic owner before upload-arena migration |
| 4 | `RawBufferVec` at `buffer_vec.rs:442` | RetiredEngine generic render resource | batch or split semantic owner before upload-arena migration |
